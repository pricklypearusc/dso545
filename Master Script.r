...
data %>%
  ...
  summarise(count = n()) %>%
  arrange(-count) %>%
  ggplot(aes(x = reorder(RequestSource,-count), y = count, fill = RequestSource, label = count))+
  geom_bar(stat = "Identity")+
  ggtitle("All Request Sources")+
  xlab("Request Source")+
  ylab("Count of Request Source")+
...
data %>%
  ...
  geom_text(size = 3.8, vjust = -1)+
  ggtitle("Request Types through Calls")+
  xlab("Request Types")+
  ylab("Count of Request Type")+
  guides(fill = FALSE)
...
data %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 16))+
  geom_text(size = 3.8, vjust = -1)+
  ggtitle("Request Types through Mobile App")+
  xlab("Request Types")+
  ylab("Count of Request Type")+
  guides(fill = FALSE)

#Request Types by Self Service
data %>%
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 16))+
  geom_text(size = 3.8, vjust = -1)+
  ggtitle("Request Types through Self Service")+
  xlab("Request Types")+
  ylab("Count of Request Type")+
  guides(fill = FALSE)
...
ggplot(data_appR, aes(x=reorder(RequestType, -app_ratio), y=app_ratio, fill = request))+
  geom_bar(stat = "identity")+
  scale_fill_gradient(midpoint = mean(data_appR$request)/4)+
  guides(fill = FALSE)+
